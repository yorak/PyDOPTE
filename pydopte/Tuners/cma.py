...
__version__ = "0.9.93.4 $Revision: 2658 $" 
#    $Date: 2011-06-01 16:11:27 +0200 (Wed, 05 Jun 2011) $
...
class Solution(np.ndarray):
    ...
    def repair(self, bounds, keep_unrepaired=True):
        ...
        if bounds is not None and not np.array_equal(bounds, [None, None]):
...
class BoundPenalty(object):
    ...
    def __call__(self, x, bounds=None):
        ...
        if bounds is None:
            bounds = self.bounds
        if bounds is None or np.array_equal(bounds, [None, None]):
...
class GenoPheno(GenoPhenoBase):
    ...
    def __init__(self, dim, scaling=None, typical_x=None, bounds=None, fixed_values=None, tf=None):
        ...
        if (self.scales == 1 and 
                ...
                (bounds is None or np.array_equal(bounds, [None, None])) and
    ...
    *pairwise mirroring* is implicite and therefore not explicitly implemented. 
    ...
    def __init__(self, x0, sigma0, inopts = {}):
        ...
        if not opts['seed']:
            ...
            opts['seed'] = 1e3 * np.random.rand() + six_decimals + 1e3 * (time.time() % 1) 
    ...
    def ask_and_eval(self, func, args=(), number=None, xmean=None, sigma_fac=1, 
        ...
        for k in xrange(int(popsize)):
            ...
            while f in (np.NaN, None):  # rejection sampling
                ...
                f = aggregation([func(x, *args) for _i in xrange(int(evaluations))])
                if nreject + 1 % 1000 == 0:
    ...
    #____________________________________________________________
    def tell(self, solutions, function_values, 
                function_values_reevaluated=None, check_points=True, copy=False):  
            ...
            if 1 < 3:  # hard clip at upper_length
                ...
                for k in idx: 
                    ...
                    if fac > 1:
                        ...
                        sigma_fac = sigma_fac * exp(np.tanh(((upper_length*fac)**2/N-1)/2) / 2)
            else:
